...
# Download WordPress
RUN curl -L "https://wordpress.org/wordpress-4.8.tar.gz" > /wordpress-4.8.tar.gz && \
    rm /var/www/html/index.html && \
    tar -xzf /wordpress-4.8.tar.gz -C /var/www/html --strip-components=1 && \
    rm /wordpress-4.8.tar.gz
 
# Download WordPress CLI
RUN curl -L "https://github.com/wp-cli/wp-cli/releases/download/v1.2.1/wp-cli-1.2.1.phar" > /usr/bin/wp && \
    chmod +x /usr/bin/wp
